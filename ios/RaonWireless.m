#import "RaonWireless.h"
#import "RSKSW/RSKSWCertManager.h"

@implementation RaonWireless
...
RCT_REMAP_METHOD(getItems,
    ...
    @catch(NSException * e) {
        NSLog(@"%@", e);
        NSLog(@"ERRORR....");
        reject(@"RaonWireless", e.userInfo.description, nil);
    }
}
...
RCT_REMAP_METHOD(getReceiveCode,
    ...
    @try {
        if (icrp == nil) {
            ...
            if (ret1 != 8) {
                reject(@"RaonWireless", @"License Not Allowed", nil);
                return;
            }
        ...
        if (ret < 0) {
            reject(@"RaonWireless", icrp.lastErrorMessage, nil);
        }
        else {
        ...
        if (ret == 1) {
            ...
            if (icrp.kmCertData != nil && icrp.kmKeyData!= nil) {
                if(![manager saveCert:icrp.kmCertData key:icrp.kmKeyData]){
                    reject(@"RaonWireless", @"Error - saveCert: key:", nil);
